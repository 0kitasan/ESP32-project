...
FloatManager::FloatManager(SensorDriver *s, MotorDriver *m, MqttLink *mq,
                           StorageDriver *st)
    : sensor(s), motor(m), mqtt(mq), storage(st), currentState(IDLE),
      lastLogTime(0), stateStartTime(0), idleInitDone(false),
...
FloatManager::State FloatManager::getState() const { return currentState; }

void FloatManager::setState(State newState) { enterState(newState); }
